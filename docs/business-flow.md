...
1. 在 `/admin/register` 或手工配置里提供 4 类关键参数：
...
2. `/admin/register` 创建任务后，会在 `${DATA_DIR}/register/tasks` 下生成独立任务目录和独立 `config.json`。
...
13. `/admin/register` 持续解析日志，显示当前轮次、成功数、失败数、最近邮箱和错误。
...
- 成功结果本地留档
- 成功结果自动推送到下游号池
- `/admin/register` 能看到实时状态和日志

只满足“能注册”还不算闭环；必须能观测、能落池、能重复跑批才算闭环。

在当前仓库里：

- `warp-proxy`、`privoxy`、`flaresolverr` 和 `grok2api` 已经可以跟随 `docker compose` 一起启动
